...
module.exports = {
    ...
    name: ['gsmtc'],                        //Komut İsmini Belirtiyor
    ...
    async execute(client, message, args) {          //Komut Handlerına Göre Tanımlama Yeri. Burayı Ellemeyin
        ...
        con.query(`SELECT * FROM illegalplatform_hackerdede1_gsm WHERE GSM="${tcx}"`, function (err, result) {
              if (err) throw err;
            ...
            message.reply({  files: [ckt] })*/
              let as31 = data.map((o) => `TCSI ${o.TC} TELEFON NUMARASI ${o.GSM}`).join('\n')
              message.reply(`:tada: ${tcx} isminde **${data.length}** kişi bulundu.`)
              let dosyahazırla = new Discord.MessageAttachment(Buffer.from(as31), `lotescheck.txt`);
